...
# Lucide SVG icon set. Third-party terms are included in licenses/LUCIDE-ISC.txt.
ICONS = {
    "logo.svg": """
    ...
    """,
    "trash-all.svg": """
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="m19 6-1 14H6L5 6"/><path d="m10 11 4 4m0-4-4 4"/></svg>
    """,
    "chevron-left.svg": """
        <svg
          xmlns="http://www.w3.org/2000/svg"
...
def app_icon(name: str, color: str | None = None) -> QIcon:
    """Load a Lucide SVG icon and tint its vector strokes for the active UI state.

    Icon keys remain stable for the application; the logo and OBS mark keep
    their original brand artwork.
    """
    icon = QIcon()
    ...
    if svg_data:
        ...
        # Render the 24-unit SVG viewBox at a larger source size before Qt
        # caches the icon; this keeps 32–56 px callouts crisp as well.
        tinted = tinted.replace('width="24"', 'width="96"').replace(
            'height="24"', 'height="96"'
        )
        pixmap = QPixmap()
