...
class ExposeListDelegate(QStyledItemDelegate):
    """Delegate pour afficher les chapitres de l'Exposé.
    Affiche:
    - Numéro de chapitre (Badge/Icone)
    - Titre du chapitre
    """
    ...
    def paint(
        ...
        # Try to extract chapter number if not provided explicitly, or just use index logic
        # Here we assume UserRole+2 might hold number, or we parse from title?
        # Typically set_chapters sets UserRole+257 = title.
        # Let's inspect library_controller later or assume we update it.
        # Ideally, we pass "num" separately.

        num = str(index.data(Qt.ItemDataRole.UserRole + 2) or "")
        if not num:
            # Fallback extraction from "Ch.X Title" format
            if "Ch." in title:
                parts = title.split()
        ...
        # Draw Number Circle/Badge
        size = 20
        y_pos = int(rect.center().y() - size / 2)
        num_rect = QRect(content_rect.left(), y_pos, size, size)
        ...
        painter.drawText(num_rect, Qt.AlignmentFlag.AlignCenter, num)
