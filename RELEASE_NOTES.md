...
Téléchargez **`ProjectOn_2.6.0_Setup.exe`** ci-dessous et installez-le par-dessus la version existante : cantiques, playlists, réglages et base sont conservés.
...
- **Modes géométriques repris, animations comprises** 🎬 : le bandeau suit aussi la **mise en page** choisie dans OBS — sous-titre, panneau latéral ou carte focus (le mode plein écran, lui, ne peut pas devenir une incrustation : il retombe sur le bandeau bas). Et il joue **l'entrée animée** de la page : fondu, glissement, zoom, flou, révélation latérale, **mot à mot** avec la pastille de source et le badge qui suivent leur propre tempo. Le **repli du texte** reproduit celui d'OBS (largeur réelle, lignes équilibrées) : les coupures tombent aux mêmes mots, aucun texte n'est rogné, et la référence respire avec le même espace que sur la page.
- **Sortie HDMI : activation à la demande** 📺 : elle ne s'ouvre plus toute seule au lancement de l'application (elle s'active depuis Réglages → Sortie HDMI), et **Échap** (ou F11) la quitte pour de bon — la sortie est éteinte, pas seulement masquée, comme pour la projection.
